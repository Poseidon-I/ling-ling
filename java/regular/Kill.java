	...
	public static void kill(GenericDiscordEvent e, String target) {
		...
		if(check.equalsIgnoreCase("i") || check.contains("ling") || check.contains("iing") || check.contains("llng") || check.contains("733409243222507670")) {
			...
			switch(i) {
				...
				case 71 -> e.reply(target + " did not have SkyBlock Level 360 and thus had low HP.  Their ego was crushed by this fact.");
				...
				case 83 -> e.reply(target + " thought that Jerry-chine Gun was the best way to get Crystals.  However, a Chad Spring Boots user yoinked their crystal.  Four times.  " + target + " then realized that their ways were wrong, and was killed by **MASTER Maxor** as they were contemplating their life choices.");
				...
				case 85 -> e.reply(target + " decided not to kill **MASTER Storm**'s Wither Guards.  Of course, " + target + " was foolishly using a pet item that was not a Shelmet, got knocked out from under the pillar, and was smited instantly by **MASTER Storm**'s lightning.");
				case 86 -> e.reply(target + " thought using Wither Cloak would save them from **MASTER Storm**'s lightning.  They were woefully wrong.  Y'all this is M7, do you really think it's this easy???");
				case 87 -> e.reply(target + " still thought that Jerry-chine Gun was the best way of getting around, and continued to use it to reach Terminals.  This time, they had their Terminal stolen every time by the Chad Spring Boots user.  They were killed in **MASTER Goldor**'s frenzy as they continued contemplating their life choices.");
				case 88 -> e.reply(target + " did not do the correct Terminal, and was kicked from the party.  In anger, " + target + " jumped into the lava, but only ended up lagging themselves out.  So now they are dead AND get no loot >:)");
				...
				case 91 -> e.reply(target + " could not do enough damage to **MASTER Necron** before he went into Tier 4 Voidgloom Seraph phase, leading to " + target + " dying due to Wither Radiation.");
				case 92 -> e.reply(target + " walked into **MASTER Necron**'s Frenzy, and immediately dropped dead.");
				case 93 -> e.reply(target + " was a complete idiot and could not tell Red from Blue.  They were ridiculed by **THE WITHER KING**, then immediately died.  Yes, being colorblind kills you in M7.  Deal with it.");
				case 94 -> e.reply(target + " was utterly confused, and let all five **WITHERED DRAGON**s spawn.  The resulting ritual resulted in the entire team's demise.  But hey, at least you still roll for Handle!");
				...
				case 98 -> e.reply(target + " forgot they were NOT playing the Tank (or Healer) class, and ran headfirst into a swarm of mobs.  The swarm of mobs won over forgetfulness and stupidity.");
				...
				case 102 -> e.reply(target + " made a SUPER BLUNDER <:blunder:1090392764635680818><:blunder:1090392764635680818><:blunder:1090392764635680818> in a chess game against " + author + " by hanging their Queen, two Rooks, two Bishops, and a Knight while simultaneously allowing Mate in 1.");
