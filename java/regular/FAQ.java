	...
	public FAQ(GuildMessageReceivedEvent e, char prefix) {
		...
		try {
			switch(message[1]) {
				...
				case "delete" -> builder.addField(":wastebasket: Why was my save randomly deleted?", "It may have been purged due to not being used at all.  Purges happen often to help save storage space and processing demands.", false);
				...
				case "cooldown" -> builder.addField(":clock2: Why is `" + prefix + "daily` and `" + prefix + "gift` cooldown not present on the Cooldowns command?", "Daily and Gift now reset at the same time every day: 00:00 UTC.  If you don't know what time it would be in your timezone, use Google.", false);
				case "loottable" -> builder.addField(":gift: What are the loot chances for the lootboxes?", "`Gift Boxes` (received as gifts from other players)\n**IF MEDAL HAS NOT BEEN OBTAINED TODAY**\n50% chance of violins, ranging between 6 times and 12 times of hourly income\n30% chance of 2-5 rice\n15% chance of 1-3 tea\n5% chance of 1 medal\n\n**OTHERWISE**\n50% chance of violins, ranging between 6 times and 12 times of hourly income\n30% chance of 2-5 rice\n20% chance of 1-3 tea\n\n`Vote Boxes` (received after claiming a vote reward)\n60% chance of violins, ranging between 3 times and 6 times of hourly income\n30% chance of 1-3 rice\n10% chance of 1 tea\n\n`???` (unobtainable - can be obtained via [ redacted ] in the future)", false);
				default -> builder.addField("You entered an invalid entry!  FAQ Entries", "Use `" + prefix + "faq [item]` to view a page in depth.\n\n`rob` `give` `luthier` `hourly` `orchestra` `rehearse` `delete` `slots` `scratch` `robchance` `leveling` `notworking` `cooldown` `loottable`", false);
			}
