		...
		if(time < (long) data.get("performCD")) {
			...
			if(num > 0.65) {
				...
				if(num > 0.45) {
					...
					data.replace("tea", (long) data.get("tea") + 4);
				...
				if(num > 0.8) {
					...
					base = -1 * income;
					e.getMessage().reply("**OOF**\nYou had a memory blank during the performance.  Your tiger mom took all your earnings, in addition to another " + Numbers.FormatNumber(income) + ":violin: for shaming yourself.").mentionRepliedUser(false).queue();
				} else if(num > 0.05) {
					base *= 0.5;
					base -= income;
					e.getMessage().reply("**OOF**\nYour performance was for a competition, and you only won Honorable Mention.  Your tiger mom Kung-Paos your chicken and takes half your earnings, in addition to another " + Numbers.FormatNumber(income) + ":violin: for being so mediocre.").mentionRepliedUser(true).queue();
				} else if(num > 0.015) {
