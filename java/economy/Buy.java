	...
	public static void ProcessBooleanUpgrade(MessageReceivedEvent e, long cost, int add, String item, String currency, JSONObject data) {
		...
		if((boolean) data.get(item)) {
			...
			if(item.equals("orchestra")) {
				...
				e.getMessage().reply("The Bank of TwoSet has noticed you, and has given you space to store violins!  You have 20 million storage for free; this can increased by buying more space using medals.").mentionRepliedUser(false).queue();
			}
		...
		try {
			...
			if(hasOrchestra && !boughtItem) {
				...
				switch(message[1]) {
					...
					case "conductor", "musicality" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("conductor"), 3, 100000), 500, 2147483647, "conductor", "violins", data);
			...
			if(!boughtItem) {
				switch(message[1]) {
					...
					case "efficiency", "ep" -> {
						if((long) data.get("efficiency") < 200) {
							ProcessUpgrade(e, Numbers.ItemCost((long) data.get("efficiency"), 1.1, 400), 0, 2147483647, "efficiency", "violins", data);
						} else {
							ProcessUpgrade(e, Numbers.ItemCost((long) data.get("efficiency") - 200, 1.2, 75962110584L), 0, 2147483647, "efficiency", "violins", data);
						}
					}
					...
					case "skill", "s" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("skills"), 2, 500), 250, 2147483647, "skills", "violins", data);
					case "lesson", "l" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("lessonQuality"), 2, 400), 200, 2147483647, "lessonQuality", "violins", data);
					case "string", "str" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("stringQuality"), 1.5, 300), 150, 2147483647, "stringQuality", "violins", data);
					case "bow", "b" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("bowQuality"), 2.5, 600), 250, 2147483647, "bowQuality", "violins", data);
					...
					case "magicfindv", "mfv" -> ProcessUpgrade(e, Numbers.ItemCost((long) data.get("magicFindViolins"), 5, 2000000), 0, 2147483647, "magicFindViolins", "violins", data);
					case "magicfindm", "mfm" -> ProcessUpgrade(e, (long) Math.pow(3, (long) data.get("magicFindMedals")), 0, 2147483647, "magicFindMedals", "medals", data);
					case "orchestra", "o" -> {
			...
			e.getMessage().reply("You have to specify an upgrade to buy, can't give you nothing.").mentionRepliedUser(false).queue();
		}
	}
}