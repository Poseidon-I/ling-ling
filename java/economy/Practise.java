		...
		if(time < (long) data.get("practiceCD")) {
			...
			if(num > 0.65) {
				...
				if(num > 0.75) {
					...
					base = -1 * income;
					e.getMessage().reply("**OOF**\nYou were caught playing Minecraft while practising.  Your tiger mom took all your earnings, in addition to another " + Numbers.FormatNumber(income) + ":violin: for being distracted.").mentionRepliedUser(false).queue();
				} else if(num > 0.015) {
					...
					data.replace("violins", violins * 0.2);
					base = 0;
				} else {
