...
public class Practise {
	public static void practise(@NotNull SlashCommandInteractionEvent e) {
		...
		if(time < (long) data.get("practiceCD")) {
			...
			if(num > 0.5) {
				e.reply("You practised for an hour and earned `" + Numbers.formatNumber(base) + "`" + Emoji.VIOLINS).queue();
				...
				if(num > 0.75) {
					...
					base = 0;
					e.reply("Oh no!  Your E String snapped while you were practising!  You had to go to the store to get it replaced, and were not able to get any practising done.  You earned `0`" + Emoji.VIOLINS + " and had to pay `" + Numbers.formatNumber(income / 100) + "`" + Emoji.VIOLINS + " for a new E String.").queue();
				} else if(num > 0.55) {
					base *= 0.9;
