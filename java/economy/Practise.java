		...
		if(time < (long) data.get("practiceCD")) {
			...
			if(num > 0.5) {
				...
				if(num > 0.75) {
					...
					e.reply(":regional_indicator_l:\nYou decided to fake your practise session.  Ling Ling caught you in the act, and fined you `" + Numbers.formatNumber((long) (violins * 0.8)) + "`" + Emoji.VIOLINS);
					data.replace("violins", (long) (violins * 0.2));
					badEvent = true;
					base = 0;
				} else {
					e.reply(":skull:\nYou dropped your violin.  How shameful.  All cooldowns except daily and gamble have had one day added to them, and you were fined `" + Numbers.formatNumber((long) (violins * 0.9)) + "`" + Emoji.VIOLINS + " for being careless.");
					time += 86400000;
