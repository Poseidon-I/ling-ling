...
public class Baldness {
	public static void baldness(GenericDiscordEvent e, String playerName) {
		try {
			...
			for(int i = 0; i < object.size(); i++) {
				...
				if(profile.get("selected").getAsBoolean()) {
					System.out.println("Detected active profile " + profile.get("cute_name").getAsString());
			...
			JsonObject skills = profile.getAsJsonObject("player_data").getAsJsonObject("experience");
			...
			long zombieLevel = Numbers.slayerLevel(slayers.getAsJsonObject("zombie").get("xp").getAsLong());
			long spiderLevel = Numbers.slayerLevel(slayers.getAsJsonObject("spider").get("xp").getAsLong());
			long wolfLevel = Numbers.slayerLevel(slayers.getAsJsonObject("wolf").get("xp").getAsLong());
			long emanLevel = Numbers.slayerLevel(slayers.getAsJsonObject("enderman").get("xp").getAsLong());
			long blazeLevel = Numbers.slayerLevel(slayers.getAsJsonObject("blaze").get("xp").getAsLong());
			long vampLevel;
			if(slayers.has("vampire")) {
			...
			long milestone = profile.getAsJsonObject("bestiary").getAsJsonObject("milestone").get("last_claimed_milestone").getAsLong();
			...
			try {
				...
				causes += "Has not unlocked all easy collections: +100\n";
				thisBaldness += 100;
			...
			try {
				if(collections.get("CHILI_PEPPER").getAsLong() < 1000000) {
			...
			try {
				if(collections.get("WILTED_BERBERIS").getAsLong() < 1000000) {
			...
			String minions = profile.getAsJsonObject("player_data").getAsJsonArray("crafted_generators").toString();
			...
			if(mp < 1697) {
				builder.addField("**__Magical Power__**", "+" + (1697 - mp) * 0.0006, false);
			...
			long hotmLevel = Numbers.hotmLevel(hotm.get("experience").getAsLong());
			double mithrilMillions = (double) (hotm.get("powder_mithril").getAsLong() + hotm.get("powder_spent_mithril").getAsLong()) / 1000000;
			double gemstoneMillions = (double) (hotm.get("powder_gemstone").getAsLong() + hotm.get("powder_spent_gemstone").getAsLong()) / 1000000;
			double glaciteMillions = (double) (hotm.get("powder_glacite").getAsLong() + hotm.get("powder_spent_glacite").getAsLong()) / 1000000;
			...
			if(total < 7000) {
				builder.addField("**__D*jo__**", "+" + (7000 - total) * 0.0001, false);
			...
			try {
				...
				if(timecharms < 7) {
					builder.addField("**__Rift Timecharms__**", "+" + (7 - timecharms) * 0.15, false);
					baldness += (7 - timecharms) * 0.15;
				}
			} catch(Exception exception) {
				builder.addField("**__Rift Timecharms__**", "+1.05", false);
				baldness += 1.05;
			}
			...
			if(!rabbits.has("aurora")) {
				causes += "Aurora Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("celestia")) {
				causes += "Celestia Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("orion")) {
				causes += "Orion Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("starfire")) {
				causes += "Starfire Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("vega")) {
				causes += "Vega Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("dante")) {
				causes += "Dante Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("einstein")) {
				causes += "Einstein Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("galaxy")) {
				causes += "Galaxy Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("king")) {
				causes += "King Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("mu")) {
				causes += "Mu Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("napoleon")) {
				causes += "Napoleon Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("omega")) {
				causes += "Omega Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("sigma")) {
				causes += "Sigma Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("zest_zephyr")) {
				causes += "Zest Zephyr Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("zeta")) {
				causes += "Zeta Rabbit: +0.04";
				thisBaldness += 0.04;
			}

			if(!rabbits.has("zorro")) {
				causes += "Zorro Rabbit: +0.04";
				thisBaldness += 0.04;
			}
			...
			if(score < 445) {
				builder.addField("**__Pet Score__**", "+" + (445 - score) * 0.002, false);
			...
			e.sendMessage("# **__Final Bald Score__**: **" + baldness + "**");
