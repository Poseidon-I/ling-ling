...
public class CheckDM {
	public static void checkDM(GenericDiscordEvent e) {
		...
		if(id.charAt(0) == '<' && id.charAt(1) == '@' && id.charAt(id.length() - 1) == '>') {
			e.sendMessage("**OI " + id + ", " + e.getAuthor().getGlobalName() + " WANTS YOU TO CHECK YOUR DMS.  DO IT NOW OR ELSE.**");
		} else {
		...
		try {
			...
			assert send != null;
			send.openPrivateChannel().complete().sendMessage("**OI " + id + ", " + e.getAuthor().getGlobalName() + " WANTS YOU TO CHECK YOUR DMS.  DO IT NOW OR ELSE.**").queue();
